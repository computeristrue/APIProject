...
// var config = `mssql://${config.db.crm.user}:${config.db.crm.password}@${config.db.crm.server}/${config.db.crm.database}`;
...
sql.query = function (sqltext) {
    return new Promise((resolve, reject) => {
        sql.queryWithParams(sqltext, null, (err,re) => {
            if(err) {
                reject(err);
                return;
            }
            /**
             * 简化查询结果，一般只有查询和修改、新增。查询的recordset为结果集，
             * 修改新增时recordset为undefined这时候返回被影响的行数信息
             **/ 
            var recordset = re.recordset;
            if(recordset == undefined){
                recordset = re.rowsAffected
            }
            resolve(recordset);
        })
    })
