...
const writeDB = async (moduleId, redis_key, write_db_id, record, id) => {
    let syncResult = 0;
    let wyFieldName = "";
    try {
...
const writeAPI = async (moduleId, redis_key, send_api_id, record, id) => {
    try {
    } catch (error) {
        log.info(error);
    } finally {
...
module.exports = async (moduleId, record, id = null) => {
    ...
    try {
        ...
        if (write_db_id) {
            syncResult = await writeDB(moduleId, redis_key, write_db_id, record, id);
        } else if (send_api_id) {
            syncResult = await writeAPI(moduleId, redis_key, send_api_id, record, id);
        }
    } catch (error) {
        log.info(error);
        syncResult = 2;
