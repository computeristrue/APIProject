...
const begin = async () => {
    try{
        ...
        const warehouseRec = await baseSql.query(crmDB,warehouseSql);
        let warehouse = {};
...
const saveByRecord = async(record,args)=>{
    ...
    if(productId && warehouseId){
        ...
        if(isExist){
            ...
            await baseSql.query(crmDB,updateSql);
        }else{
            ...
            await baseSql.query(crmDB,insertSql);
        }
    }
}
...
const isExistByProductAndWarehouse = async (productId,warehouseId)=>{
    ...
    const records = await baseSql.query(crmDB,selectSql);
