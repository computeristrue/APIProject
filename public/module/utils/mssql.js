...
// var k3Config = `mssql://${cfg.db.u8.user}:${cfg.db.u8.password}@${cfg.db.u8.server}/${cfg.db.u8.database}`;
 ...
 */
sql.queryWithParams = function (dbInfo,sqltext, params, func) {
    try {
        var connection = new mssql.ConnectionPool(dbInfo, function (err) {
            if (err)
                func(err);
            else {
                var request = new mssql.Request(connection);
 ...
 * @param {function} func 回调函数 共有三个参数 error:错误消息 recordsets:查询的结果 affected:影响的行数
 */
sql.query = function (dbInfo,sqltext) {
    return new Promise((resolve, reject) => {
        sql.queryWithParams(dbInfo,sqltext, null, (err,re) => {
            if(err) {
                reject(err);
                return;
            }
            var records = [];
            records = re ? re.recordset : [];
            resolve(records);
        });
    })
};
 ...
 */
sql.queryViaStreamWithParams = function (dbInfo,sqltext, params, func) {
    try {
        dbInfo.stream = true;

        mssql.connect(dbInfo, function (err) {
            if (err)
                func.error(err);
            else {
 ...
 */
sql.queryViaStream = function (dbInfo,sqltext, func) {
    sql.queryViaStreamWithParams(dbInfo,sqltext, null, func);
};
